...
export default function CommandCenter() {
  ...
  const metrics = [
    ...
    { label: 'Arus kas bersih', value: s.netCash, icon: Wallet, entity: null, positive: amount(s.netCash) >= 0 },
    { label: 'Piutang berjalan', value: s.receivableOutstanding, icon: Clock3, entity: 'receivables' },
  ];
  return <div className="overview">
    <section className="overview-welcome"><img src="/art/flame-spectrum.jpg" alt=""/><div><span className="overline">KEUANGAN DALAM KENDALI</span><h2>Angka yang jelas.<br/>Langkah yang terarah.</h2><p>Pantau arus kas, tinjau pengajuan, dan lanjutkan pekerjaan Anda.</p><button className="welcome-action" onClick={() => open('income')}>Catat pemasukan <ArrowUpRight size={16}/></button></div><span className="welcome-edition">RUN FINANCE / WORKSPACE</span></section>
    <div className="section-eyebrow"><span>Ringkasan keuangan</span><span>Sesuai filter aktif · IDR</span></div>
    <div className="finance-kpis" aria-label="Indikator keuangan utama">{metrics.map(({ label, value, icon: Icon, entity, positive }) => <button key={label} className={`finance-kpi ${positive ? 'positive' : ''}`} onClick={() => entity ? open(entity) : setView('analytics')}><span className="kpi-top"><span>{label}</span><Icon size={18}/></span><strong>{money.format(amount(value))}</strong><span className="kpi-bottom">{entity === 'receivables' ? 'Saldo berjalan · cakupan brand' : 'Periode transaksi yang dipilih'}<ArrowUpRight size={14}/></span></button>)}</div>
    ...
    <div className="overview-metrics-row">
      <section className="finance-card balance-card">
        ...
        <p className="muted">Saldo berjalan · mengikuti company dan brand, tanpa batas tanggal</p>
      ...
      <section className="finance-card action-card"><div className="section-heading"><h3>Perlu ditinjau</h3><Clock3 size={17}/></div><strong className="pending-number">{amount(s.pendingApproval)}<span>pengajuan menunggu</span></strong><div className="review-row"><span>Total pengajuan dalam filter</span><strong>{money.format(amount(s.budgetRequested))}</strong></div><div className="review-row"><span>Hutang berjalan</span><strong>{money.format(amount(s.payableOutstanding))}</strong></div><button className="btn primary" onClick={() => setView('approval')}>Buka approval <ArrowRight size={14}/></button></section>
      <section className="finance-card target-card gauge-card"><div className="section-heading"><h3>Capaian omzet</h3></div><div className="gauge-wrap">{amount(s.omzetTarget) > 0 ? <ProgressRing value={achievement} color="#E85002" size={140} glow bare/> : <div className="target-empty"><strong>—</strong><span>Target belum ditetapkan</span></div>}</div><div className="target-numbers"><span>{money.format(amount(s.omzetReal))}<small>Realisasi</small></span><span>{money.format(amount(s.omzetTarget))}<small>Target</small></span></div><button className="text-action" onClick={() => open('omzet')}>Lihat detail omzet <ArrowRight size={14}/></button></section>
    </div>
