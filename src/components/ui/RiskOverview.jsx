...
export default function RiskOverview({ summary: s }) {
  ...
  const ewsMetrics = [
    ...
    { label: 'Margin kas indikatif', value: pct.format(s.npm || 0), color: npmColor, statusLabel: npmD.label, arti: 'Selisih omzet dan pengeluaran kas. Bukan laba bersih akuntansi; belum mencakup penyesuaian akrual.', rumus: npmD.rumus },
  ...
  const ratioKeys = [null, 'cashOutRatio', 'npm', 'cashConversion', 'receivableRisk', 'payableRisk'];
  ratioKeys.forEach((key, index) => {
    if (key && s[key] == null) Object.assign(ewsMetrics[index], { value: '—', color: 'teal', statusLabel: 'Belum dapat dihitung', arti: 'Data pembagi pada bulan berjalan belum tersedia atau bernilai nol.' });
  });

  return <details className="finance-card risk-disclosure"><summary>Indikator risiko keuangan <span>Saldo berjalan · rasio bulan berjalan</span></summary><div className="metric-grid">{ewsMetrics.map(m => <MetricCard key={m.label} {...m}/>)}</div></details>;
}
